...
const IncomingOrder = () => {
  ...
  const filteredOrders = orders.filter(order => {
    return order.customer_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      order.customer_email.toLowerCase().includes(searchQuery.toLowerCase()) ||
      order.customer_phone.includes(searchQuery) ||
      order.pickup_date.includes(searchQuery);
  });
    ...
    <div className="overflow-x-auto mb-40 sm:rounded-lg">
      ...
      <table className="ml-72 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            <th scope="col" className="px-4 py-3">Names</th>
            <th scope="col" className="px-4 py-3">Customer Email</th>
            <th scope="col" className="px-4 py-3">Phone Number</th>
            <th scope="col" className="px-4 py-3">Product & Quantity</th>
            <th scope="col" className="px-4 py-3">Pickup Date</th>
        ...
        <tbody>
          {filteredOrders.map((order) => (
            ...
            >
              ...
              <td className="px-4 py-4">{order.pickup_date}</td>
